// Shared preset-33 help panel. Native code computes ownership, layout and animation.
...
(function(O){
 ...
 function Menu(rom,input){
  ...
  this.rom=rom;this.leases=[];this.audio=[];
  ...
  for(const [address,n]of [[0x8018fdc0,4],[0x8022a970,8],[0x800c4b28,2],[0x80187020,1]])regions.push({address,bytes:new Uint8Array(n),writable:true});
 ...
 Menu.prototype.create=function(slot,preset,width,height){if(!Number.isInteger(slot)||slot<0||slot>=14||preset!==33||![width,height].every(n=>Number.isInteger(n)&&n>=1&&n<=2048))fail('Map menu supports preset 33, valid owner slots, and current image dimensions.');const m=this.machine;m.put(CTX+0x834,width,2);m.put(CTX+0x836,height,2);this.invoke(0x80236920,[slot,preset]);return this.owner(slot);};
 Menu.prototype.advance=function(mask){if(!Number.isInteger(mask)||mask<0||mask>65535)fail('Menu update requires a declared current controller mask.');this.audio=[];this.machine.put(0x800c4b28,mask,2);this.invoke(0x80237620);return this.audio.slice();};
