...
(function(OB64){
 ...
 function Launch(input,rom){
  ...
  if(input.previewParty===true){if(!OB64.cutscenePreservedStage)stop('Preview party services are unavailable.','preview-party-contract');if(input.preservedStage||input.sceneMode!==2||input.world.mapKind!==0||input.world.eventState!==0||dv.getUint32(decoded.length-4)!==0xff000008)stop('Preview party startup requires terminal class eight and its declared world.','preview-party-contract');this.stage=new OB64.cutscenePreservedStage(rom,input);this.stage.attach(this);}
 }
