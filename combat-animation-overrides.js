// OB64 combat body-animation Normal/Blocked and exact-route selector overrides.
window.OB64 = window.OB64 || {};
(function() {
  'use strict';
  var D = OB64.COMBAT_ANIMATION_OVERRIDE_DATA;
  if (!D || D.schemaVersion !== 3) throw new Error('OBSO-v3 combat animation data was not loaded');
  var L = D.layout, V1 = D.legacyV1, V2 = D.legacyV2;
  var NO_MATCH_SELECTOR = 0x28;
  ...
  function cloneEntries(a) { return (a||[]).map(function(x){var row={classId:x.classId,actionId:x.actionId,normalSelector:x.normalSelector,blockedSelector:x.blockedSelector};if(Number.isInteger(x.bodyFlags))row.bodyFlags=x.bodyFlags;return row;}); }
  function key(x) { return x.classId+':'+x.actionId+':'+(Number.isInteger(x.bodyFlags)?x.bodyFlags:'*'); }
  function sortEntries(a) { return a.sort(function(x,y){var xb=Number.isInteger(x.bodyFlags)?x.bodyFlags:-1,yb=Number.isInteger(y.bodyFlags)?y.bodyFlags:-1;return x.classId-y.classId||x.actionId-y.actionId||xb-yb;}); }
  ...
  function validateLogicalEntries(entries) {
    ...
    for(var i=0;i<entries.length;i++){
      ...
      var allowed={classId:1,actionId:1,normalSelector:1,blockedSelector:1,bodyFlags:1};for(var f in x)if(!allowed[f])throw new Error('Override '+i+' has unknown field '+f);
      ...
      var bodyFlags=Object.prototype.hasOwnProperty.call(x,'bodyFlags')?Number(x.bodyFlags):null;
      if(bodyFlags!==null&&(!Number.isInteger(bodyFlags)||bodyFlags<0||bodyFlags>3))throw new Error('Override '+i+' bodyFlags must be 0, 1, 2, or 3');
      if(bodyFlags===null){
      ...
      var row={classId:cid,actionId:aid,normalSelector:normal,blockedSelector:blocked};if(bodyFlags!==null)row.bodyFlags=bodyFlags;
      var k=key(row);if(seen[k])throw new Error('Duplicate combat animation override for class '+hex(cid)+' / action '+hex(aid)+(bodyFlags===null?'':' / body flags '+bodyFlags));seen[k]=true;
      out.push(row);
    }
  ...
  function compileTable(entries) {
    ...
    var records=entries.map(function(x){var c=classInfo(x.classId),exactBody=Number.isInteger(x.bodyFlags);return {sourceArt:c.sourceArt,rawOwner:c.rawOwner,action:x.actionId,flags:exactBody?2:0,original:0,normal:x.normalSelector,blocked:x.blockedSelector,bodyFlags:exactBody?x.bodyFlags:0};});
    records.sort(function(a,b){return a.sourceArt-b.sourceArt||a.rawOwner-b.rawOwner||a.action-b.action||((a.flags?0:1)-(b.flags?0:1))||a.bodyFlags-b.bodyFlags||a.original-b.original;});
    var table=new Uint8Array(1024);for(var i=0;i<records.length;i++){var o=i*8,r=records[i];table[o]=r.sourceArt;table[o+1]=r.rawOwner;table[o+2]=r.action;table[o+3]=r.flags;table[o+4]=r.original;table[o+5]=r.normal;table[o+6]=r.blocked;table[o+7]=r.bodyFlags;}
  ...
  function lookupRecords(records,sourceArt,rawOwner,action,original,rawMode,bodyFlags){if(rawMode!==0&&rawMode!==1&&rawMode!==2)return original;for(var i=0;i<(records||[]).length;i++){var r=records[i];if(r.sourceArt!==sourceArt||r.rawOwner!==rawOwner||r.action!==action)continue;if((r.flags&1)&&r.original!==original)continue;if((r.flags&2)&&r.bodyFlags!==bodyFlags)continue;return rawMode===2?r.blocked:r.normal;}return original;}

  var RETAIL_HOOK=bytes(D.retailHookHex),V3_HOOK=bytes(D.ownedHookHex),V3_BOOT=bytes(D.bootstrapHex),V3_RETURN=bytes(D.selectorReturnPostimageHex),RETAIL_RETURN=bytes(D.selectorReturnPreimageHex);
  var V2_HOOK=bytes(V2.ownedHookHex),V2_BOOT=bytes(V2.bootstrapHex),V1_HOOK=bytes(V1.ownedHookHex),V1_BOOT=bytes(V1.bootstrapHex),CARRIERS=D.carrierPatches.map(function(x){return {data:x,pre:bytes(x.preimageHex),post:bytes(x.postimageHex)};});
  ...
  function inspectOwned(z,version) {
    ...
    for(var i=0;i<128;i++){
      ...
      var r={sourceArt:table[o],rawOwner:table[o+1],action:table[o+2],flags:table[o+3],original:table[o+4],normal:table[o+5],blocked:version===1?table[o+5]:table[o+6],bodyFlags:version===3?table[o+7]:0};
      var malformedFlags=version===3?(r.flags<0||r.flags>3):(r.flags!==0&&r.flags!==1);
      if(malformedFlags||(!(r.flags&1)&&r.original!==0)||(!(r.flags&2)&&r.bodyFlags!==0)||(r.bodyFlags<0||r.bodyFlags>3)||(version===1&&(table[o+6]||table[o+7]))||(version===2&&table[o+7]))return {ok:false,reason:'OBSO record '+i+' is malformed'};
      var sk=[r.sourceArt,r.rawOwner,r.action,r.flags?0:1,r.bodyFlags,r.original].map(function(v){return String(v).padStart(3,'0');}).join(':');if(previous!==null&&sk<previous)return {ok:false,reason:'OBSO records are not in canonical order'};previous=sk;
      var dk=[r.sourceArt,r.rawOwner,r.action,r.flags,(r.flags&1)?r.original:0,(r.flags&2)?r.bodyFlags:0].join(':');if(seen[dk])return {ok:false,reason:'OBSO table contains a duplicate record'};seen[dk]=true;raw.push(r);
      var matches=D.classes.filter(function(c){return c.sourceArt===r.sourceArt&&c.rawOwner===r.rawOwner;});
      if(matches.length!==1||!actionInfo(r.action)||(r.flags&1)||(!(r.flags&2)&&(!selectorInfo(matches[0],r.normal)||!selectorInfo(matches[0],r.blocked))))advanced.push(r);
      else {var logicalRow={classId:matches[0].id,actionId:r.action,normalSelector:r.normal,blockedSelector:r.blocked};if(r.flags&2)logicalRow.bodyFlags=r.bodyFlags;logical.push(logicalRow);}
    }
    var template=version===1?V1.moduleTemplateHex:(version===2?V2.moduleTemplateHex:D.moduleTemplateHex);if(!immutableModuleMatches(mod,template))return {ok:false,reason:'OBSO v'+version+' immutable module code mismatch'};
    if(!advanced.length&&version===3){var rebuilt=buildModule(logical).module;if(!same(rebuilt,mod))return {ok:false,reason:'OBSO module is not editor-owned v3'};}
  ...
  function classify(z) {
    ...
    if(same(h,V3_HOOK)&&same(boot,V3_BOOT)&&same(ret,V3_RETURN)&&carrierState(z,true)){var v3=inspectOwned(z,3);if(v3.ok){v3.kind='owned-v3';return v3;}return {kind:'foreign',reason:v3.reason};}
    if(same(h,V2_HOOK)&&same(boot,V2_BOOT)&&same(ret,V3_RETURN)&&carrierState(z,true)){var v2=inspectOwned(z,2);if(v2.ok){v2.kind='owned-v2';return v2;}return {kind:'foreign',reason:v2.reason};}
  ...
  function initialize(rom) {
    ...
    if(supported){var c=classify(rom.z64);state.laneState=c.kind;if(c.kind==='owned-v1'||c.kind==='owned-v2'||c.kind==='owned-v3'){state.advanced=!!c.advanced;state.readOnly=!!c.advanced;state.diagnostic=c.advanced?'Advanced OBSO v'+c.version+' table uses exact-selector or unmappable records and is preserved read-only.':'';state.baseline=cloneEntries(c.logical);state.desired=cloneEntries(c.logical);state.rawRecords=c.raw;state.loadedVersion=c.version;}else if(c.kind==='foreign'){state.readOnly=true;state.diagnostic=c.reason;state.disabledReason='Attack Animation override lanes are foreign or partial: '+c.reason;}}
  ...
  function setEntry(state,entry){if(!state.supported||state.readOnly)throw new Error(state.disabledReason||state.diagnostic);var next=cloneEntries(state.desired),found=false,normalized={classId:Number(entry.classId),actionId:Number(entry.actionId),normalSelector:Number(entry.normalSelector),blockedSelector:Number(entry.blockedSelector)};if(Number.isInteger(entry.bodyFlags))normalized.bodyFlags=Number(entry.bodyFlags);for(var i=0;i<next.length;i++)if(key(next[i])===key(normalized)){next[i]=normalized;found=true;}if(!found)next.push(normalized);state.desired=validateLogicalEntries(next);refresh(state);return state;}
  function removeEntry(state,cid,aid,bodyFlags){if(!state.supported||state.readOnly)throw new Error(state.disabledReason||state.diagnostic);var exact=Number.isInteger(bodyFlags);state.desired=state.desired.filter(function(x){return x.classId!==Number(cid)||x.actionId!==Number(aid)||(exact?x.bodyFlags!==Number(bodyFlags):Number.isInteger(x.bodyFlags));});refresh(state);return state;}
  function exactEntry(state,cid,aid,bodyFlags){for(var i=0;i<(state&&state.desired||[]).length;i++){var row=state.desired[i];if(row.classId===Number(cid)&&row.actionId===Number(aid)&&row.bodyFlags===Number(bodyFlags))return row;}return null;}
  function v3Writes(module) {var writes=[{offset:L.hookZ64,bytes:V3_HOOK,label:'selector hook'},{offset:L.bootstrapZ64,bytes:V3_BOOT,label:'selector bootstrap'},{offset:L.moduleZ64,bytes:module,label:'selector module/table'},{offset:L.selectorReturnZ64,bytes:V3_RETURN,label:'selector-return raw-mode hook'}];CARRIERS.forEach(function(x){writes.push({offset:x.data.z64,bytes:x.post,label:x.data.label});});return writes;}
  ...
  function adopt(rom){var s=rom.combatAnimationOverrides;if(!s)return;s.baseline=cloneEntries(s.desired);s.laneState=s.desired.length?'owned-v3':'retail';s.loadedVersion=s.desired.length?3:null;s.advanced=false;s.readOnly=false;s.diagnostic='';s.dirty=false;}
  ...
  function collisionOwner(rom){var s=rom&&rom.combatAnimationOverrides;if(!s||(!s.dirty&&s.laneState!=='owned-v1'&&s.laneState!=='owned-v2'&&s.laneState!=='owned-v3'&&!s.desired.length))return null;return {id:'combat-animation-overrides',name:'Combat Attack Animation Overrides',regions:regions()};}
  ...
  function modalRows(state,def,classId) {
    var cid = Number(classId), overrides = {}, seen = {}, out = [];
    (state && state.desired || []).forEach(function(row) {
      if (row.classId === cid && !Number.isInteger(row.bodyFlags)) {
        overrides[row.actionId] = row;
      }
    });
    [def && def.b43Raw, def && def.b45Raw, def && def.b47Raw]
      .forEach(function(value) {
        var aid = Number(value || 0), override = overrides[aid];
        if (!aid || seen[aid] || !actionInfo(aid)) return;
        seen[aid] = true;
        var vanilla = override ? null : vanillaPairForLiveAction(cid, def, aid);
        var pair = override || vanilla || {
          normalSelector: NO_MATCH_SELECTOR,
          blockedSelector: NO_MATCH_SELECTOR
        };
        out.push({
          classId: cid,
          actionId: aid,
          ...
          overridden: !!override,
          mappingSource: override ? 'override' : (vanilla ? 'vanilla' : 'fallback'),
          assignmentRequired: !override && !vanilla
        });
      });
    (state && state.desired || []).forEach(function(row) {
      if (row.classId !== cid || Number.isInteger(row.bodyFlags) || seen[row.actionId]) {
        return;
      }
    ...
    return out.sort(function(a,b) { return a.actionId - b.actionId; });
  }
  function applyLiveAttackChange(state,def,classId,field,newActionId) {
    var fields = { b43Raw: 1, b45Raw: 1, b47Raw: 1 };
    if (!def || !fields[field]) throw new Error('Live class attack field is unavailable');
    var nextAction = Number(newActionId);
    if (!Number.isInteger(nextAction) || nextAction < 0 || nextAction > 255) {
      throw new Error('Live class attack must be a u8');
    }
    var previous = Number(def[field] || 0);
    def[field] = nextAction;
    var existing = null, hasRouteAssignment = false;
    if (nextAction && state && state.supported && !state.readOnly) {
      for (var i = 0; i < state.desired.length; i++) {
        var row = state.desired[i];
        if (row.classId !== Number(classId) || row.actionId !== nextAction) continue;
        hasRouteAssignment = true;
        if (!Number.isInteger(row.bodyFlags)) existing = row;
      }
    }
    var vanilla = nextAction
      ? vanillaPairForLiveAction(classId, def, nextAction) : null;
    var pair = existing || vanilla;
    return {
      classChanged: previous !== nextAction,
      overrideAdded: false,
      previousAction: previous,
      newAction: nextAction,
      normalSelector: pair ? Number(pair.normalSelector) : null,
      blockedSelector: pair ? Number(pair.blockedSelector) : null,
      mappingSource: existing ? 'override' : (vanilla ? 'vanilla' : 'fallback'),
      requiresAnimationSelection: !!nextAction && !!state && state.supported &&
        !state.readOnly && !pair && !hasRouteAssignment
    };
  }
  function describeEntry(x){var c=classInfo(x.classId),a=actionInfo(x.actionId),route=Number.isInteger(x.bodyFlags)?' body flags '+Math.floor(x.bodyFlags/2)+'/'+(x.bodyFlags&1):'';return (c?c.name:hex(x.classId))+' '+(a?a.name:hex(x.actionId))+' '+hex(x.actionId)+route+' -> Normal '+hex(x.normalSelector)+', Blocked '+hex(x.blockedSelector);}

  OB64.combatAnimationOverrides={data:D,initialize:initialize,classify:classify,validateLogicalEntries:validateLogicalEntries,compileTable:compileTable,buildModule:buildModule,lookupRecords:lookupRecords,sha256:sha256,selectorOptions:selectorOptions,classInfo:classInfo,actionInfo:actionInfo,setEntry:setEntry,removeEntry:removeEntry,exactEntry:exactEntry,refresh:refresh,diff:diff,count:function(s){return s.desired.length;},capacity:L.capacity,noMatchSelector:NO_MATCH_SELECTOR,prepareExport:prepareExport,applyPlan:applyPlan,adopt:adopt,patchRegions:regions,collisionOwner:collisionOwner,collectProjectPayload:collectProjectPayload,validateProjectPayload:validateProjectPayload,applyProjectPayload:applyProjectPayload,liveRanks:liveRanks,liveRankStatus:liveRankStatus,vanillaSelectorForLiveAction:vanillaSelectorForLiveAction,vanillaPairForLiveAction:vanillaPairForLiveAction,modalRows:modalRows,applyLiveAttackChange:applyLiveAttackChange,describeEntry:describeEntry};
})();
