...
const cutsceneSpriteSource = fs.readFileSync(
  path.join(EDITOR, 'cutscene-sprites.js'), 'utf8');
...
assert(source.includes("'Transparent Preview: ' +"));
assert(source.includes("togglePreviewBackground(ui);"));
assert(source.includes("if (kind === 'icon') toolbar.appendChild(previewBackgroundButton"));
assert(source.includes("nativePngCanvas(words, width, height)"));
assert(source.includes("'Changes transparent pixels in editor previews only."));
assert(animationSource.includes("data-art-scroll-key', 'animations:sequence:' + animation.key"));
assert(animationSource.includes("weaponChildOrdinal, 'transparent'"));
assert(animationSource.includes("cached.backgroundMode === backgroundMode"));
assert(animationSource.includes('var surfaces = animationWebmSurfaces('));
assert(animationSource.includes("getContext('2d', { alpha: true })"));
assert(animationSource.includes('Recording one transparent animation loop'));
assert(animationSource.includes('appendPreviewBackgroundButton(bar, ui, rerender);'));
...
assert(animationSource.includes("'Cutscene actors'"));
assert(animationSource.includes("selectField('Actor Art Source')"));
assert(animationSource.includes("selectField('Physical Pose')"));
assert(animationSource.includes("selectField('Appearance')"));
assert(animationSource.includes("button('Export Frame PNG'"));
assert(animationSource.includes("button('Export Sequence WebM'"));
assert(animationSource.includes("button('Copy Complete Frame'"));
assert(animationSource.includes('applySelectedCopy(true);'));
assert(animationSource.includes("caption: 'Animation preview · 30 ticks/sec'"));
assert(animationSource.indexOf("caption: 'Animation preview · 30 ticks/sec'") >
  animationSource.indexOf("'Selected sprite only'"));
assert(animationSource.includes('OB64.cutsceneSprites.actorSequence('));
assert(cutsceneSpriteSource.includes('actorSequence: actorSequence'));
...
assert(animationSource.includes('OB64.animationSequences.setFrameTicks('));
...
assert(cssSource.includes('.animation-frame-tick-input {'));
...
assert(cssSource.includes('.art-transparent-button.white-preview {'));
...
assert(cssSource.includes('.animation-sprite-copy-modal {'));
assert(cssSource.includes('.animation-sprite-copy-preview-grid.with-sequence-preview {'));
