...
(async()=>{
 ...
 const rendered=OB64.cutsceneSprites.framesForPreview(spriteState,runtime.states[0]);
 assert.equal(Object.keys(rendered).length,fixture.expected.length,JSON.stringify(spriteState.errors));
 for(const expected of fixture.expected){const a=runtime.states[0].actors.find(a=>a.slot===expected.slot),native=OB64.cutsceneRuntime.decodeNativeActorState(a.nativeActorState);
  assert.equal(native.decoderMode,0);assert.equal(a.bodyPoseProgram,null);
  assert.equal(a.artSourceId,'cutscene-art-bank:'+a.bank);assert(!a.poseId.startsWith('body-pose:'));
  assert.equal(a.source.recordHex,fixture.input.capturedSnapshot.value.slots[a.slot].recordHex);
 ...
 console.log(JSON.stringify({status:'pass',sourceHashes,actors:fixture.expected.length,spriteFrames:Object.keys(rendered).length,spriteErrors:spriteState.errors,invalidInputs:mutations.length,cancellation:'pass',asyncParity:'pass',outcome:runtime.outcome,retainedStateBytes:runtime.retainedStateBytes,limits:runtime.limits}));
