...
(async () => {
  ...
  prepare.focus();prepare.click();let modal=document.body.querySelector('[role="dialog"]');assert(modal);assert(modal.textContent.includes('Prepare Sprite Library Sequence'));
  ...
  prepare.click();modal=document.body.querySelector('[role="dialog"]');dom.findText(modal,'Import Sequence').click();assert.strictEqual(changes,1);
