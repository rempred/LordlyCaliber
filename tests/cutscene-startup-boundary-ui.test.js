...
(async()=>{
 ...
 const unsupported=state.catalog.getScene('rom-director:01F450D0');state.selectedSceneId=unsupported.sceneId;const unsupportedDocument=await OB64.cutsceneUI.loadScene(rom,state,unsupported);
 checkBoundary(unsupported,unsupportedDocument,'ROM startup does not implement terminal resource class 2 and its caller setup.');
 // Changing selection must not carry the other resource's error into playback.
 const supported=state.catalog.getScene('rom-custom-lz:01FA4D0A');state.selectedSceneId=supported.sceneId;const document=await OB64.cutsceneUI.loadScene(rom,state,supported);assert.equal(state.runtimeByAssetId[supported.assetId].outcome,'modeled-termination');let text=render(supported,document);assert(!text.includes('Director playback unavailable'));assert(!text.includes('terminal resource class 2'));assert.equal(state.ui.playButton.textContent,'Play');
 ...
 assert(state.sourceErrors['runtime-context:'+unsupported.assetId].includes('terminal resource class 2'));results.push({assetId:supported.assetId,runtime:true,passes:235,selectionRecovery:true,sameResourceRecovery:true,staleErrorCleared:true});
